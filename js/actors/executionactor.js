...
const findChildElement = (stage, elementid) => {
    return (stage.children || []).find(ch => ch.type === 'element' && ch.id === elementid);
};

const ensureChildElement = (stage, elementid) => {
    let child = findChildElement(stage, elementid);
    if (!child) {
...
const isStageComplete = (stage) => {
	...
	if (child.type === 'element') return child.status === 'EXECUTED';
	if (child.type === 'stage') return isStageComplete(child);
	return false;
    });
};

const recomputeAncestorCompletion = (pipeline, topStageId) => {
    const stage = pipeline.stages[topStageId];
    if (!stage) return;
    if (isStageComplete(stage)) stage.status = 'executed';
};
...
const scanInterruptedPath = (stage, path) => {
	...
	if (child.type === 'element') {
	    if (child.status === 'RUNNING') return childPath;
	}
...
const executionbehavior = (state, message) => {
	...
	case EXECUTIONMESSAGETYPES.STAGE_STATE: {
		...
		if (Array.isArray(message.state.children)) {
			...
			if (child.type === 'element') {
			    return newElementNode(child.id, child.status || 'WAITING');
			}
			if (child.type === 'stage') {
	...
	case EXECUTIONMESSAGETYPES.ELEMENT_STATE: {
		...
		recomputeAncestorCompletion(pipeline, path[0]);
            }
