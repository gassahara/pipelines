...
var STOREMAILBOX = [];
var STOREMAILBOX = STOREMAILBOX;

// DBACTOR's own consumer registry
var DBACTORCONSUMERS = {};
var DBACTORCONSUMERS = DBACTORCONSUMERS;
...
var DBBEHAVIOR = function(ENV, MESSAGE) {
  ...
  switch (MESSAGE.TYPE) {
    case MESSAGETYPES.STORE:
      ...
      try {
        ...
        if (SERIALIZED.length > MAXENTRYBYTES) {
          ...
          STORESEND(MESSAGE.SENDER, MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT, { ERROR: 'value too large' }, MESSAGE.TAG, 'DBACTOR');
          return ENV;
        }
      } catch (E) {
        STORESEND(MESSAGE.SENDER, MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT, { ERROR: E.message || String(E) }, MESSAGE.TAG, 'DBACTOR');
        return ENV;
      }
      var KEYS = Object.keys(STORE);
      ...
      if (PERSISTED) STORESEND(MESSAGE.SENDER, MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT, { RESULT: true }, MESSAGE.TAG, 'DBACTOR');
      else STORESEND(MESSAGE.SENDER, MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT, { ERROR: 'persist failed' }, MESSAGE.TAG, 'DBACTOR');
      return ENV;
    ...
    case MESSAGETYPES.RESTORE:
      ...
      STORESEND(MESSAGE.SENDER, MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT, { RESULT: RESTOREDVALUE }, MESSAGE.TAG, 'DBACTOR');
      return ENV;

    case MESSAGETYPES.LIST:
      logdebug(ENV, '[DBACTOR]', 'ACTION LIST COUNT:', Object.keys(STORE).length);
      STORESEND(MESSAGE.SENDER, MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT, { RESULT: Object.keys(STORE) }, MESSAGE.TAG, 'DBACTOR');
      return ENV;
    ...
    case MESSAGETYPES.DELETE:
      ...
      if (PERSISTEDDEL) STORESEND(MESSAGE.SENDER, MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT, { RESULT: true }, MESSAGE.TAG, 'DBACTOR');
      else STORESEND(MESSAGE.SENDER, MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT, { ERROR: 'persist failed' }, MESSAGE.TAG, 'DBACTOR');
      return ENV;

    default:
      logwarn(ENV, '[DBACTOR]', 'UNKNOWN ACTION:', MESSAGE.TYPE);
      STORESEND(MESSAGE.SENDER, MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT, { ERROR: '[DBACTOR] unknown message type' }, MESSAGE.TAG, 'DBACTOR');
      return ENV;
  }
};
...
function DBSTORE(KEY, VALUE) {
  var TAG = GENERATETAG();
  var DBRESULTTYPE = MESSAGETYPES.DBRESULT || MESSAGETYPES.DBRESULT;
