...
### 9.7 EXECUTION — ONE AFFECTED FILE PER ITERATION
EXECUTION is the source-modification mode. Its normal chat unit is **one affected file per execution cycle**.
For each file cycle:
1. Resolve `@file`.
2. Resolve the corresponding blueprint section.
3. Resolve only that file's instruction operations.
4. Display/materialize the minimum source context required for exact application and verification.
5. Apply the instructions.
6. Verify the file's postconditions.
7. Record the result using trace references.
8. Close the file cycle.
9. Expose the next file/instruction continuation target.
